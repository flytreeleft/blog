...
本例所要求的便是求解任意两个序列的最长公共子序列，并打印其长度及其其中的任意一个序列。
<!--more-->
...
- 若 $$x_i=y_j$$ ，则 $$z_k=x_i=y_j$$ ，且 $$Z_{k-1}=\{ z_1, z_2, ..., z_{k-1} \}$$ 是 $$X_{i-1}=\{ x_1, x_2, ..., a_{i-1} \}$$ 与 $$Y_{j-1}=\{ y_1, y_2, ..., b_{j-1} \}$$ 的一个最长公共子序列
- 若 $$x_i \neq y_j$$ ，则若 $$z_k \neq x_i$$ ，那么， $$Z_k$$ 是 $$X_{i-1}$$ 与 $$Y_j$$ 的一个最长公共子序列。注：此时 $$z_k$$ 不一定等于 $$y_j$$ ，但前面的推论仍然包含等于或不等于的情况
- 若 $$x_i \neq y_j$$ ，则若 $$z_k \neq y_j$$ ，那么， $$Z_k$$ 是 $$X_i$$ 与 $$Y_{j-1}$$ 的一个最长公共子序列。注：此时 $$z_k$$ 不一定等于 $$x_i$$ ，但前面的推论依然包含等于或不等于的情况

根据以上推论可进一步推断以下递归描述是与其等效的：
...
因此，如果用二位数组 $$f[i][j]$$ 表示序列 $$X_i$$ 和 $$Y_j$$ 的最长公共子序列的长度，那么根据前面的最长公共子序列的公式，便可相应地推出求解最长公共子序列的长度的公式，即：
...
这样， $$f[i][j]$$ 中的最大值便是最长公共子序列的长度，而根据该数组回溯，便可找到最长公共子序列。

以序列 $$X_i=\{ A, B, C, B, D, A, B \}$$ 和 $$Y_j=\{ B, D, C, A, B, A \}$$ 为例，可以通过下图了解求解最长公共子序列长度的整个过程：
...
typedef struct _LCSLen {
    int value; // 最长公共子序列的长度
    LCSLenDir dir; // 长度求值基于哪个方向的结果
} LCSLen;

// Note：二维数组作为参数时，必须指定第二维的长度
void lcs_search_dynamic_programming(LCSLen lcs_len[][MAX_SEQ_LEN], int seq_x[], int seq_y[], int seq_x_len, int seq_y_len) {
    // 将(i,0)的单元格全部置为0
...
int main(int argc, char *argv[]) {
    ...
    // int seq_x[] = {'A', 'C', 'C', 'G', 'G', 'T', 'C', 'G', 'A', 'G', 'T', 'G', 'C', 'G', 'C', 'G', 'G', 'A', 'A', 'G', 'C', 'C', 'G', 'G', 'C', 'C', 'G', 'A', 'A'};
    int seq_x_len = sizeof(seq_x) / sizeof(seq_x[0]);

    int seq_y[] = {'B', 'D', 'C', 'A', 'B', 'A'};
    // int seq_y[] = {'G', 'T', 'C', 'G', 'T', 'T', 'C', 'G', 'G', 'A', 'A', 'T', 'G', 'C', 'C', 'G', 'T', 'T', 'G', 'C', 'T', 'C', 'T', 'G', 'T', 'A', 'A', 'A'};
